...
ALPHA = 0.1 # alpha for dirichlet distribution

def dirichlet_split(y, alpha, k):
    # dirichlet
    sy = sorted(set(y))
    label_distribution = np.random.dirichlet([alpha]*k, len(sy))
    # class-id
    class_idcs = []
    for l in sy:
...
if __name__ == "__main__":
    ...
    client_idcs = dirichlet_split(y, alpha=ALPHA, k=N)
