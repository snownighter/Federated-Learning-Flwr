...
import numpy as np
def save_excsv(a, b):
    sh = [np.array(a).shape, np.array(b).shape]
    print('weights_result',len(a),len(a[0]),len(a[0][0]))
    print('aggregate_weights:',len(b),len(b[0]),len(b[0][0]))
    #pd.DataFrame(sh).to_csv('shape_print.csv')
    #pd.DataFrame(a).to_csv('weights_results.csv')
    #pd.DataFrame(b).to_csv('aggregate_weights.csv')
...
class FedFlan(FedAvg):
    ...
    def __init__(
        ...
        *,
        ...
        Implementation based on Snighter (2023.01) and FedAvg
